const PvcfoamPlate = require("../../../Model/Allproductschema/Rigidsignplates/Pvcfoamplates");
...
const CreateAcpPlate = async (req, res, next) => {
  try {
    ...
    const existingProduct = await PvcfoamPlate.findOne({ sku });
    if (existingProduct) {
      return next(new AppErr("Product with this SKU already exists", 400));
    }

    const newProduct = new PvcfoamPlate({
      name,
      categoryId,
      sku,
      description,
...
const CalculateAcpPlatePrice = async (req, res, next) => {
  try {
    ...
    // Find the correct product configuration
    const product = await PvcfoamPlate.findOne({ "configurations.plateType": plateType });
...
const GetAllAcpPlates = async (req, res, next) => {
  try {
    const products = await PvcfoamPlate.find();
    res.status(200).json({
      status: true,
      data: products,
    });
...
const GetSingleAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    const product = await PvcfoamPlate.findById(id);
...
const UpdateAcpPlate = async (req, res, next) => {
  try {
    ...
    const updatedProduct = await PvcfoamPlate.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
const DeleteAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    await PvcfoamPlate.findByIdAndDelete(id);
