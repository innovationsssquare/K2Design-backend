const AcrylicPlate = require("../../../Model/Allproductschema/Rigidsignplates/Acrylicplates");
...
const CreateAcpPlate = async (req, res, next) => {
  try {
    ...
    const existingProduct = await AcrylicPlate.findOne({ sku });
    if (existingProduct) {
      return next(new AppErr("Product with this SKU already exists", 400));
    }

    const newProduct = new AcrylicPlate({
      name,
      categoryId,
      sku,
      description,
...
const CalculateAcpPlatePrice = async (req, res, next) => {
  try {
    ...
    // Find the correct product configuration
    const product = await AcrylicPlate.findOne({ "configurations.plateType": plateType });
...
const GetAllAcpPlates = async (req, res, next) => {
  try {
    const products = await AcrylicPlate.find();
    res.status(200).json({
      status: true,
      data: products,
    });
...
const GetSingleAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    const product = await AcrylicPlate.findById(id);
...
const UpdateAcpPlate = async (req, res, next) => {
  try {
    ...
    const updatedProduct = await AcrylicPlate.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
const DeleteAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    await AcrylicPlate.findByIdAndDelete(id);
