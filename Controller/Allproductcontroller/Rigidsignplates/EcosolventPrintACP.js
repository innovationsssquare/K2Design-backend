const AcpPlate = require("../../../Model/Allproductschema/Rigidsignplates/EcosolventPrintACP");
...
const CreateAcpPlate = async (req, res, next) => {
  try {
    ...
    const existingProduct = await AcpPlate.findOne({ sku });
    if (existingProduct) {
      return next(new AppErr("Product with this SKU already exists", 400));
    }

    const newProduct = new AcpPlate({
      name,
      categoryId,
      sku,
      description,
...
const GetAllAcpPlates = async (req, res, next) => {
  try {
    const products = await AcpPlate.find();
    res.status(200).json({
      status: true,
      data: products,
    });
...
const GetSingleAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    const product = await AcpPlate.findById(id);
...
const UpdateAcpPlate = async (req, res, next) => {
  try {
    ...
    const updatedProduct = await AcpPlate.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
const DeleteAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    await AcpPlate.findByIdAndDelete(id);
