const NightlowPlate = require("../../../Model/Allproductschema/Rigidsignplates/Nightglowplates");
...
const CreateAcpPlate = async (req, res, next) => {
  try {
    ...
    const existingProduct = await NightlowPlate.findOne({ sku });
    if (existingProduct) {
      return next(new AppErr("Product with this SKU already exists", 400));
    }

    const newProduct = new NightlowPlate({
      name,
      categoryId,
      sku,
      description,
...
const CalculateAcpPlatePrice = async (req, res, next) => {
  try {
    ...
    // Find the correct product configuration
    const product = await NightlowPlate.findOne({ "configurations.plateType": plateType });
...
const GetAllAcpPlates = async (req, res, next) => {
  try {
    const products = await NightlowPlate.find();
    res.status(200).json({
      status: true,
      data: products,
    });
...
const GetSingleAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    const product = await NightlowPlate.findById(id);
...
const UpdateAcpPlate = async (req, res, next) => {
  try {
    ...
    const updatedProduct = await NightlowPlate.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
const DeleteAcpPlate = async (req, res, next) => {
  try {
    const { id } = req.params;
    await NightlowPlate.findByIdAndDelete(id);
