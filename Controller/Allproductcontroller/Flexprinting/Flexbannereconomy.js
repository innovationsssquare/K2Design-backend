const FlexBannerPrinteconomy = require("../../../Model/Allproductschema/Flexprinting/Flexbannereconomy");
...
const CreateFlexBannerPrint = async (req, res, next) => {
  try {
    ...
    const existingProduct = await FlexBannerPrinteconomy.findOne({ sku });
    if (existingProduct) {
      return next(new AppErr("Product with this SKU already exists", 400));
    }

    const newProduct = new FlexBannerPrinteconomy({
      name,
      categoryId,
      sku,
      description,
...
const CalculateFlexBannerPrintPrice = async (req, res, next) => {
  try {
    ...
    const product = await FlexBannerPrinteconomy.findOne({
...
const GetAllFlexBannerPrints = async (req, res, next) => {
  try {
    const products = await FlexBannerPrinteconomy.find();
    res.status(200).json({
      status: true,
      data: products,
    });
...
const GetSingleFlexBannerPrint = async (req, res, next) => {
  try {
    const { id } = req.params;
    const product = await FlexBannerPrinteconomy.findById(id);
...
const UpdateFlexBannerPrint = async (req, res, next) => {
  try {
    ...
    const updatedProduct = await FlexBannerPrinteconomy.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
const DeleteFlexBannerPrint = async (req, res, next) => {
  try {
    const { id } = req.params;
    await FlexBannerPrinteconomy.findByIdAndDelete(id);
